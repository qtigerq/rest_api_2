...
const axios = require('axios');     //cliente HTTP para fazer requisições - https://axios-http.com/docs/intro
const postsService = require('../service/postsService');

const generate = () => {            //função para gerar strings randomicas de 20 caracteres
...
test('Should get posts', async function(){
    ...
    const post1 = await postsService.savePost({ title: generate(), content: generate()});       //await para esperar inserir antes de continuar o teste, senao pode dar resultado errado
...
test('Should save a post', async function(){
    ...
    const data = { title: generate(), content: generate()};                                     //cria um objeto post
...
test('Should update a post', async function(){
    ...
    const post = await postsService.savePost({ title: generate(), content: generate()});         //Insere um post aleatório no banco de dados

    post.title = generate();                                                                     //Modifica o title do post (mas ainda não no banco)
